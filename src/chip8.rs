...
impl Chip8 {
    ...
    fn op_7xxx(&mut self) {
        ...
        self.register[self.x()] = self.register[self.x()].wrapping_add(self.nn());
        self.pc += 2;
    }
    ...
    fn op_8xx4(&mut self) {
        ...
        self.register[self.x()] = self.register[self.x()].wrapping_add(self.register[self.y()]);
        self.pc += 2;
    }
    ...
    fn op_8xx6(&mut self) {
        ...
        self.register[self.x()] = self.register[self.y()] >> 1;
        self.pc += 2;
    }
    ...
    fn op_8xx7(&mut self) {
        ...
        self.register[self.x()] = (self.register[self.y()]).wrapping_sub(self.register[self.x()]);
        self.pc += 2;
    }
    ...
    fn op_dxxx(&mut self) {
        ...
        for row in 0..self.opcode & 0x000F {
            ...
            for column in 0..8 {
                ...
                if font_row & (0x80 >> column) != 0 {
                    if self.display[(vx + column + ((vy + row) * 64)) as usize] {
                        self.register[15] = 1;
                    }
                    self.display[(vx + column + ((vy + row) * 64)) as usize] ^= true;
                }
            }
        }
        self.draw_flag = true;
    ...
    fn op_fx07(&mut self) {
        self.register[self.x()] = self.delay_timer;
        self.pc += 2;
    }
    ...
    fn op_fx15(&mut self) {
        self.delay_timer = self.register[self.x()];
        self.pc += 2;
    }

    // FX18: set sound_timer to V[X]
    fn op_fx18(&mut self) {
        self.sound_timer = self.register[self.x()];
        self.pc += 2;
    }

    // FX1E: add V[X] to I
    fn op_fx1e(&mut self) {
        self.index += u16::from(self.register[self.x()]);
        self.pc += 2;
    }

    // FX29: set I to the location ofthe sprite for the character in V[X]
    fn op_fx29(&mut self) {
        self.index = u16::from(self.register[self.x()] * 5);
        self.pc += 2;
    }
    ...
    fn op_fx33(&mut self) {
        self.memory[self.index as usize] = self.register[self.x()] / 100;
        self.memory[(self.index + 1) as usize] = (self.register[self.x()] % 100) / 10;
        self.memory[(self.index + 2) as usize] = self.register[self.x()] % 10;
        self.pc += 2;
    }
    ...
    fn op_fx55(&mut self) {
        for i in 0..=self.x() {
            self.memory[i + self.index as usize] = self.register[i];
        }
        self.pc += 2;
    }
    ...
    fn op_fx65(&mut self) {
        for i in 0..=self.x() {
            self.register[i] = self.memory[i + self.index as usize];
        }
        self.pc += 2;
    }
}
